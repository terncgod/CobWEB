	...
	def output(self):
		...
		# if verbose 2
			...
			if data != None:
				...
				info('No information found (on shodan) for this email... sending the strongest troups to find this IP/IP\'s on the internet...')
				if self.file != None:
					self.file.write('%s- No information found (on shodan) for this email... sending the strongest troups to find this IP/IP\'s on internet...')
			if self.pwned:
				headers = ''
				data = Pwned(self.email).search()
				if data is None:
		...
		# if verbose 3					
			...
			if data != None:
				...
				info('No information found (on shodan) for this email... sending the strongest troup to find this IP/IP\'s on internet...')
				if self.file != None:
					self.file.write('%s- No information found (on shodan) for this email... sending the strongest troup to find this IP/IP\'s on internet...')
			if self.pwned:
				headers = ''
				data = Pwned(self.email).search()
				if data is None:
