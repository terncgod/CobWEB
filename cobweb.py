...
class cobweb(object):
	...
	def main(self):
		...
		try:
			opts,args = getopt.getopt(sys.argv[1:],'d:s:o:v:r:hb',
				['domain=','source=','osint=','breach-check','verbose=','help','report='])
		...
		for o,a in opts:
			...
			if o in ('-b','--breach-check'):self.breach=True
		...
		for email in self.listEmail:
			...
			if ip != ([] or None):
				...
				if len(ips) >=2:
					info("Found multiple IP\'s for this email...")
				PPrint(ips,email,self.verbose,self.breach,self.report).output()
			else:more('No Information found for %s'%(email))
		if self.report != None:
